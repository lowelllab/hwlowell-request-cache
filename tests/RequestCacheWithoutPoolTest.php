...
class RequestCacheWithoutPoolTest extends TestCase
{
    /**
     * @var RequestCache
     */
    ...
    protected function setUp(): void
    {
        ...
        CacheConfig::setRedisPoolConfig(['enabled' => false]);

        //Mock Redis facade
        $redisMock = Mockery::mock('alias:Illuminate\\Support\\Facades\\Redis');
        $this->redisConnectionMock = Mockery::mock();
        $redisMock->shouldReceive('connection')->andReturn($this->redisConnectionMock);
    ...
    public function testGetFromRedis()
    {
        ...
        $this->redisConnectionMock->shouldReceive('get')->with($key)->andReturn($jsonData);
    ...
    public function testGetReturnsNullWhenNoCache()
    {
        ...
        $key = $this->requestCache->generateKey($gateway, $params);
        $this->localCacheMock->shouldReceive('get')->with($key)->andReturn(null);
        $this->redisConnectionMock->shouldReceive('get')->with($key)->andReturn(null);
    ...
    public function testSet()
    {
        ...
        $this->redisConnectionMock->shouldReceive('setex')->with($key, $expire, $jsonData)->andReturn(true);
    ...
    public function testSetWithTags()
    {
        ...
        $this->redisConnectionMock->shouldReceive('setex')->with($key, $expire, $jsonData)->andReturn(true);
        $this->redisConnectionMock->shouldReceive('sadd')->with($tagKey, $key)->andReturn(true);
        $this->redisConnectionMock->shouldReceive('expire')->with($tagKey, $expire + 3600)->andReturn(true);
        $this->localCacheMock->shouldReceive('set')->with($key, $data, $expire)->andReturn(true);
    ...
    public function testDelete()
    {
        ...
        $key = $this->requestCache->generateKey($gateway, $params);
        $this->redisConnectionMock->shouldReceive('del')->with($key)->andReturn(1);
    ...
    public function testClearGateway()
    {
        ...
        $this->redisConnectionMock->shouldReceive('command')->with('SCAN', ['0', 'MATCH', $pattern, 'COUNT', 1000])->andReturn(['0', $keys]);
        $this->redisConnectionMock->shouldReceive('del')->with($keys)->andReturn(2);
    ...
    public function testClearTags()
    {
        ...
        $this->redisConnectionMock->shouldReceive('smembers')->with($tagKey1)->andReturn(['key1', 'key2']);
        $this->redisConnectionMock->shouldReceive('smembers')->with($tagKey2)->andReturn(['key3']);
        $this->redisConnectionMock->shouldReceive('del')->with($tagKey1)->andReturn(1);
        $this->redisConnectionMock->shouldReceive('del')->with($tagKey2)->andReturn(1);
        $this->redisConnectionMock->shouldReceive('del')->with($keys)->andReturn(3);
    ...
    public function testRememberWithCacheMiss()
    {
        ...
        $this->localCacheMock->shouldReceive('get')->with($key)->andReturn(null);
        $this->redisConnectionMock->shouldReceive('get')->with($key)->andReturn(null);
        $this->redisConnectionMock->shouldReceive('set')->with('laravel_local_cache:lock:' . $key, Mockery::any(), 'EX', 5, 'NX')->andReturn(true);
        $this->redisConnectionMock->shouldReceive('eval')->with(Mockery::any(), 1, 'laravel_local_cache:lock:' . $key, Mockery::any())->andReturn(1);
        $this->redisConnectionMock->shouldReceive('setex')->with($key, 300, json_encode($expectedData))->andReturn(true);
    ...
    {
        $today = date('Y-m-d');
        $this->redisConnectionMock->shouldReceive('get')->with('laravel_local_cache:stats:hits')->andReturn(10);
        $this->redisConnectionMock->shouldReceive('get')->with('laravel_local_cache:stats:misses')->andReturn(5);
        $this->redisConnectionMock->shouldReceive('get')->with('laravel_local_cache:stats:hits:' . $today)->andReturn(2);
        $this->redisConnectionMock->shouldReceive('get')->with('laravel_local_cache:stats:misses:' . $today)->andReturn(1);
    ...
    public function testSetWithRedisException()
    {
        ...
        $key = $this->requestCache->generateKey($gateway, $params);
        $this->redisConnectionMock->shouldReceive('setex')->andThrow(new \Exception('Redis error'));
